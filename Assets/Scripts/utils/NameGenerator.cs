...
public class NameGenerator : MonoBehaviour
{
    ...
    private string GenerateRandomName(List<string> namePrefixes, List<string> nameSuffixes) 
    {
        string name = namePrefixes[random.Next(0, namePrefixes.Count)] + nameSuffixes[random.Next(0, nameSuffixes.Count)];
        return name;
    }

    private string GetNameByRealm(RealmType realm)
    {
        switch (realm)
        {
            case RealmType.FIRE:
